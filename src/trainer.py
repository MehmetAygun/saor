...
from utils.metrics import Metrics, MeshEvaluator, calc_pose_apprx_error
...
class Trainer:
    ...
    def __init__(self, cfg, run_dir, gpu=None, rank=None, world_size=None):
        ...
        self.pose_history_dir = os.path.join(self.run_dir, 'pose_history')
        if not os.path.exists(self.pose_history_dir):
            os.mkdir(self.pose_history_dir)

        self.device = get_torch_device(gpu, verbose=True)
        cfg["model"]['device'] = self.device
        cfg["model"]["n_epoches"] = cfg["training"].get("n_epoches")
        self.train_loader, self.val_loader, self.test_loader = create_train_val_test_loader(cfg, rank, world_size)
        if hasattr(self.train_loader.dataset, 'n_classes'):
            cfg["model"]["n_classes"] = self.train_loader.dataset.n_classes
        ...
        self.model.n_epoches = self.n_epoches
        ...
        self.val_metrics = Metrics('loss_val', 'pose_val', log_file=self.run_dir / 'val_metrics.tsv', append=append)
    ...
    def load_from(self, cfg):
        ...
        if tag is not None:
            ...
            if resume is not None:
                ...
                if cfg['training'].get('init_rend'):
                    print ('Re initialize rendering')
                    self.model._init_rend_predictors(**cfg['model']['rend_predictor'])
                    self.model = self.model.to(self.device)

                if 'init_art' in cfg['training'] and cfg['training']['init_art']:
                    print ('Re initialize articulation')
                    self.model._init_articulation(**cfg['model']['articulation'])
                    self.model = self.model.to(self.device)

                if 'freeze_def' in cfg['training'] and cfg['training']['freeze_def']:
                    for p in self.model.deform_field.parameters():
                        p.requires_grad = False
                    print ('Deformation is frozen!!')
                try:
                    if 'finetune' in cfg['training']:
                        if cfg['training']['finetune']:
                            print ('not loading optimizer as doing finetuning')
                    else:
                ...
                self.cur_lr = self.scheduler.get_last_lr()[0]
                # self.model.pose_temp = 1 - (min((self.epoch_start / 500.), 0.99))

                self.model.pose_temp = 1 - (min((self.epoch_start / self.n_epoches), 0.99))
        ...
        for epoch in range(self.epoch_start, self.n_epoches + 1):
            ...
            for batch, (images, _labels) in enumerate(self.train_loader, start=1):
                if batch < batch_start:
                    continue
                self.run_single_batch_train(images, _labels)
                if cur_iter % self.train_stat_interval == 0 and self.is_master:
                    self.log_train_metrics(cur_iter, epoch, batch)
                    #save pose information to debug
                    pose_hist_path = os.path.join(self.pose_history_dir, '{}.npy'.format(cur_iter))
                    np.save(pose_hist_path, self.model.pose_history)
    ...
    def run_single_batch_train(self, images, labels=[None]):
        start_time = time.time()
        self.model.train()
        self.optimizer.zero_grad()
        loss, pred = self.model(torch_to(images, self.device, non_blocking=self.multi_gpu), labels.to(self.device))
    ...
    def run_val(self):
        ...
        for images, labels in self.val_loader:
            loss, pred = model(torch_to(images, self.device), labels.to(self.device), return_meshes=False)
            ...
            # B = labels.shape[0]
            # indices = self.model._pose_proba.max(0)[1]
            # azim_max = self.model.azim_preds.view(self.model.n_poses, B)[indices, torch.arange(B)]
            # pose_accs = calc_pose_apprx_error(azim_max, labels).item()
            #
            # self.val_metrics.update('pose_val', pose_accs, N=1)
    ...
    def log_visualizations(self, cur_iter):
        ...
        viz_fake_labels = torch.zeros(viz_imgs.shape[0]).to(self.device)
        rec = self.model(self.viz_samples, viz_fake_labels, debug=True).permute(1, 0, 2, 3, 4)
        ...
        rec = self.model(self.viz_samples, viz_fake_labels)[1]
    ...
    def save_metric_plots(self):
        ...
        viz_fake_labels = torch.zeros(self.viz_samples['imgs'].shape[0]).to(self.device)
        self.rec_logger.save(self.model(self.viz_samples, viz_fake_labels)[1])
