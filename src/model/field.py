...
class Field(nn.Module):
    ...
    def __init__(self, n_units=N_UNITS, n_layers=N_LAYERS, latent_size=None, positional_encoding=False,
    in_ch=3, out_ch=3, zero_last_init=True):
        super().__init__()
        NU, NL = n_units, n_layers

        self.positional_encoding = positional_encoding
        if self.positional_encoding:
            self.embedder, self.embedder_dim = get_embedder(10)
            in_ch = self.embedder_dim
    ...
    def forward(self, x, latent=None):
        if latent is not None:
            ...
            B, N, C = x.shape
            return self.mlp(x.view(B*N,C)).view(B,N,-1)  # BN3
        else:
            if self.positional_encoding:
                x = self.embedder(x)
...
class ProgressiveField(nn.Module):
    def __init__(self, inp_dim, name, powers, milestones=None, **kwargs):
        ...
        positional_encoding = kwargs.pop('positional_encoding', False)

        self.field = Field(NU, NL, latent_size=self.latent_size, positional_encoding=positional_encoding)
