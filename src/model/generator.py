...
class NewGiraffeGenerator(nn.Module):
    """Neural renderer class from https://github.com/autonomousvision/giraffe"""

    def __init__(self, n_features=128, inp_dim=128, img_size=64, out_dim=3, min_features=32, **kwargs):
        ...
        self.use_rgb_skip = kwargs.pop('use_rgb_skip', False)
        self.use_norm = kwargs.pop('use_norm', True)
        upsample_feat, upsample_rgb = kwargs.pop('upsample_feat', 'nn'), kwargs.pop('upsample_rgb', 'bilinear')
        self.shared_encoder = kwargs.pop('shared_encoder', True)
        assert len(kwargs) == 0
        ...
        self.conv_layers = nn.ModuleList([conv3x3(n_features, max(n_features // 2, min_features))] + seq)
        ...
        if self.use_norm:
            self.norms = nn.ModuleList([nn.GroupNorm(8, n_ch_fn(i + 1)) for i in range(self.n_blocks)])
    ...
    def forward(self, inp):
        ...
        _,_,h,w = net.shape
        n_upsample_step = self.n_blocks - int(log2(h))

        if self.use_rgb_skip:
            rgb = self.upsample_rgb(self.conv_rgb[0](net))

        for idx, layer in enumerate(self.conv_layers):
            if idx < n_upsample_step:
...
class ProgressiveGiraffeGenerator(nn.Module):
    def __init__(self, inp_dim, powers, milestones, **kwargs):
        ...
        self.shared_encoder = kwargs['shared_encoder']
        ...
        if self.shared_encoder:
            ...
            self.regressor = conv1x1(inp_dim, self.latent_size)
            self.generator = NewGiraffeGenerator(n_features=n_features, inp_dim=self.latent_size, **kwargs)
    ...
    def forward(self, x):
        ...
        if self.act_idx < self.n_powers:
            ...
            if self.shared_encoder:
                ...
                B, C, H, W = latent_final.shape
                mask = torch.zeros_like(latent_final, device=device)
                mask[:, :p , : , :] = torch.ones(B, p, H, W, device=device)
                latent_final = mask * latent_final
